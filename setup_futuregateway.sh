...
#   proper ssh key exchange and without prompting passwords
...
EOF
    if [ "$SETUP_FLAG" -ne 0 ]; then
         ...
         out "passed ("$PKGMGR")" 0 1 && RES=0 || RES=1
             ...
             RES=0 || RES=1 
             [ $RES -eq 0 ] && out "done" 0 1
        fi

    if [ $RES -ne 0 ]; then
        out "failed" 0 1
        err "Failing $SETUP_SERVICE host checking; command, ssh output and error files below"
            err "CMD: "
            err "$CMD"
        err "SSH Output:"
        err "$(cat $SSH_OUT)"
...
# FutureGateway component such as: setup_config.sh and setup_commons.sh files
